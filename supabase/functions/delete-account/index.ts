// Supabase Edge Function — deletes the CALLING user's own account, and every
// piece of app data tied to them (posts, journal entries, attendance,
// messages, etc.) — a full wipe, matching how admin-delete-user handles
// admin-initiated removals, so the two flows behave consistently.
// Invoked from the app as: supabase.functions.invoke("delete-account")
// (the user's session is attached automatically by the Supabase JS client)
//
// NOTE: the data-wipe logic below is intentionally duplicated in
// admin-delete-user's index.ts rather than shared via an import, since the
...
//
// This must run server-side: deleting an auth.users row (and bypassing RLS
// to clean up data across every table) requires the service_role key, which
...
serve(async (req) => {
  ...
  const result = await deleteUserCompletely(adminClient, user.id);
